...
async def crash() -> dict[str, str]:
    """Throw — should produce an issue in GlitchTip / thaqafa-backend."""
    raise RuntimeError("[thaqafa] /_debug/crash test exception")


@router.get("/log-warning")
async def log_warning() -> dict[str, str]:
    """Emit a WARNING — should land in GlitchTip via the loguru sink."""
    logger.bind(probe="glitchtip").warning("[thaqafa] /_debug/log-warning probe")
