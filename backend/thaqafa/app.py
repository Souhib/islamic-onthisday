...
from thaqafa.api.errors import BaseError
from thaqafa.api.middleware import LoggingMiddleware, RequestIDMiddleware, SecurityMiddleware
from thaqafa.api.rate_limit import configure_rate_limit
from thaqafa.api.routes import auth as auth_route
from thaqafa.api.routes import bookmarks as bookmarks_route
from thaqafa.api.routes import debug as debug_route
from thaqafa.api.routes import events as events_route
from thaqafa.api.routes import health as health_route
from thaqafa.api.routes import lessons as lessons_route
from thaqafa.api.routes import observances as observances_route
from thaqafa.api.routes import people as people_route
from thaqafa.api.routes import recent as recent_route
from thaqafa.api.routes import today as today_route
from thaqafa.api.routes import upcoming as upcoming_route
from thaqafa.database import dispose_engine, init_engine
from thaqafa.logger_config import configure_logger
from thaqafa.observability import configure_sentry
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    is_production = settings.thaqafa_env == "production"

    @asynccontextmanager
    async def lifespan(_: FastAPI) -> AsyncIterator[None]:
        await init_engine(settings)
        logger.bind(env=settings.thaqafa_env, port=settings.port).info("api_started")
        try:
            yield
        finally:
