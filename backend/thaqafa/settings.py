...
``THAQAFA_ENV`` selector, and the matching ``.env.{env}`` file holds the actual
...
def _resolve_env_files() -> tuple[str, ...]:
    ...
    Resolution order for the environment selector:
        1. ``THAQAFA_ENV`` shell variable (CI / Docker)
        2. ``THAQAFA_ENV`` value in the top-level ``.env``
    ...
    """
    env = os.environ.get("THAQAFA_ENV") or dotenv_values(_BACKEND_DIR / ".env").get("THAQAFA_ENV") or "development"
